...
module Seh
  class Event < OpenStruct
    # @param event_template - optionally provide a Seh::EventTemplate for this Event.  If an EventTemplate is provided, this Event may not declare additional types or stages.
    def initialize event_template=nil
      super()
      @state = :ready
      @targets = Set.new
      @start_callbacks = []
      ...
      if event_template
        @types = event_template.types
        @stages = event_template.stages
        @stage_decision_blocks = event_template.stage_decision_blocks
      else
        @types = Set.new
        @stages = Set.new
    ...
    def type *event_types
      raise "Event#type is disallowed because this Event was constructed with an EventTemplate" if @event_template
    ...
    def add_stage new_stage, &stage_decision_block
      raise "Event#add_stage is disallowed because this Event was constructed with an EventTemplate" if @event_template
    ...
    def run_stage_callbacks
      @event_template.start_callbacks.each { |block| block.call self } if @event_template
      @start_callbacks.each { |block| block.call self }
      @stages.each do |stage|
        @event_template.stage_callbacks[stage].each { |block| block.call self } if @event_template
        @stage_callbacks[stage].each { |block| block.call self } if @stage_callbacks.key? stage
      end
      @event_template.finish_callbacks.each { |block| block.call self } if @event_template
